...
`HOUSE_AGENTS_ROOT` moves it. It is regenerated by `mise run examples
--write` and compared with a fresh render by `mise run test`, so the table
above is checked, not described.
